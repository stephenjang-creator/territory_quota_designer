...
pipeline-coverage target (3×). All $ are USD ACV. The default plan is a 36-rep team
(10 Enterprise / 15 Mid-Market / 11 SMB) carrying a **$7.0M/quarter ($28M/year)**
standardized company target.
...
assess_territory("R-125")          # → drill into one for the full picture
```

`coverage_gaps` returns the **11 SMB reps** at ~2.33x to 2.49x pipeline coverage, each
with the exact pipeline gap (target × quota minus available). `assess_territory("R-125")`
shows why: a standardized SMB AE quota of $110K/quarter needs $330K of pipeline at
3×, but the book holds only ~$273K, a **$57K capacity gap**. It's not a carve
mistake: the whole SMB segment holds $2.81M vs the $3.50M its quotas need at 3×. An
...
Re-carves to the stiffer target and diffs. Reps covered drops from **25 to 0**: at 5×,
even the surplus Enterprise and Mid-Market segments can't pack every book that deep,
and the total capacity gap balloons from $0.69M to **$6.69M**. Returns the coverage
floor, the gap, and the full below-target list. Powers "how much pipeline coverage
can we actually demand before the model breaks".
...
same role, same number), the company target rises from $7.00M to **$7.81M/quarter**,
and each affected rep's pipeline coverage drops (a richer quota on the same thin
book). Returns the before/after quota and coverage per affected rep. Powers "we want
to pay SMB more, can the segment support the quota that implies?".
...
Total comp, cost-of-sale, and the top/bottom earners at 90% (cost-of-sale ~25.6%).
Call `comp_scenario()` with no argument for the 0.85 / 1.0 / 1.10 table. Comp is
OTE-anchored and **annual** (base = a slice of OTE, on-target variable earned in full
at 100%), so at-plan cost-of-sale sits near **25%** and ticks only slightly with
attainment (26.0% at 85%, 25.0% at plan, 25.3% at 110% as the accelerator kicks in).
The table shows it honestly.
...
capacity gap is **identical** ($688K either way): SMB is short no matter how you
carve, and an assignment can't invent pipeline. The work-back win is *fairness*: it
raises the coverage floor (worst rep 2.22× to 2.33×) and keeps books compact (off-home
0.72 to 0.26) by spreading the SMB shortfall evenly across regions instead of starving
one rep to over-fill another. Includes a ready-to-paste markdown table and the
per-segment pipeline-vs-required breakdown.

---

### 6. "Is R-125 set up to fail, or is the segment just short on pipeline?"

```
assess_territory("R-125")
```
...
**coverage** is 2.48× vs the 3× target. So it isn't that R-125's quota is unfair;
...
**Enterprise AE** is absorbed by the segment's pipeline surplus: reps-covered goes 25 to 26
and the company target moves $7.00M to $7.28M. **Two SMB AEs** push the already-short SMB
segment further underwater: both land uncovered (reps-covered holds at 25 of 38) and the
coverage floor drops 2.33× to 1.95×, so the tool flags SMB as the segment you can't hire
into without sourcing more pipeline. Returns the company-target delta, reps covered
before/after, the coverage floor, and the segment's pipeline vs. required. The dashboard
does the same interactively: add reps in the roster (bottom section) and watch the
capacity bars.
...
whatif_segment_override("SMB", quota_to_ote=3.21)    # or model one lever directly
```

`recommend_actions` returns the plan's issues, each with an **applyable settings delta**.
For the SMB shortfall it offers three fixes: **re-tag 102 Mid-Market accounts** into SMB
($701K of pipeline, every SMB rep then clears 3×, the source segment stays covered),
**lower the SMB quota multiple to 3.21×**, or **accept a 2.41× coverage target for SMB**,
plus a safe hire plan (5 Enterprise + 6 Mid-Market AE), a comp-cost check, and the global
target/multiple at which every rep clears. `whatif_segment_override` models one lever
directly: cutting SMB to 3.21× makes the segment coverable and drops the company target
from $7.00M to $6.77M/quarter. The dashboard renders the same actions with an **Apply**
button on each.
